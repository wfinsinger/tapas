...
Since v0.1.2, a GAM-modeled trend can be used as well for the *decomposition* step, though the procedure is a little bit more tedious, as it is not implemented in the `peak_detection()` wrapper function:
- if the data was *resampled*, use the `tapas2mgcv()` function to extract the data stored in the list generated with the `pretreatment_data()` function. Then model the trend using the `mgcv::gam()` function, and translate the `mgcv::gam()`-generated list using the `mgcv2tapas()` function. Then move on to the *screening* step.
- if the data wasn't *resampled*, simply omit the `tapas2mgcv()` step.
