...
#     proxy         =>  Set proxy="VariableName" to determine threshold for one single variable. In this case, a 
#                       Figure will be generated and saved to the "./Figures_SJH/" directory on the hard drive and
#                       a list will be returned with the threshold data for the analysed proxy.
#                       With the beta-version default option (proxy=NULL), the analysis uses all variables that are in the output
#                       from the SeriesDetrend() function; Figures for each variable will be generated and saved to the "./Figures_SJH/" directory
#                       on the hard drive; a list will be returned with the threshold data for the last proxy only.
#     t.lim         =>  allows defining a portion of the time series. Should be t.lim=c(older age limit, younger age limit).
#                       With t.lim=NULL (as per default) the analysis will be performed using the entire timeseries.
#     thresh.value  =>  Determines the location of the threshold as the percentile of the Gaussian Model of the noise component
#     noise.gmm     =>  Determines which of the two GMM components should be considered as the noise component. By default noise.gmm=1.
#     gm.local      =>  Determines if GMMs are determined based on the entire timeseries (gm.local=F), or if local thresholds
#                         should be calculated (gm.local=T)
...
#     smoothing.yr  =>  determines the smoothing-window width to smooth the threshold values (if gm.local=T)
#     span.sm       =>  determines the smoothing of the threshold values. If span.sm=NULL, the span is calculated based on smoothing.yr
#
#  ***************************************************************************


SeriesThresh <- function(series=NA, proxy=NULL, t.lim=NULL, thresh.yr=1000,
  ...
  # If using a global threshold with 2 components ####
  if (gm.local == F) {
    ...
    plot(ageI, SNIsm.pos, type = "l", xlim = x.lim, col = "black",
         ylim = c(0, 1.2*max(SNIsm.pos)), axes  =  F,
         xlab  =  "Age (cal yrs BP)", ylab  =  "smoothed SNI")
  ...
  # If using a local threshold with 2 components ####
  if (gm.local == T) {
  ...
  if (gm.local == T) {
    out1 <- structure(list(proxy = proxy, ages.thresh = ageI,
                           ...
                           SNIsm = cbind(SNIsm.pos, SNIsm.neg),
